...
LOG="${LOG:-submissions/09_run_ppo_rm.log}"
touch "$LOG"
exec > >(tee -a "$LOG") 2>&1

echo "[09_run_ppo_rm] started: $(date -Is)"
echo "[09_run_ppo_rm] pwd=$(pwd)"
PPO_ACTOR_PATH="${PPO_ACTOR_PATH:-/workspace/checkpoints/sft_merged}"
echo "[09_run_ppo_rm] actor_path=${PPO_ACTOR_PATH}"
...
echo "[09_run_ppo_rm] finished: $(date -Is)"
